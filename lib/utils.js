...
module.exports = {
  ...
  createJwt: function(req, res, user) {
    var configData = waterlock.config.jsonWebTokens;
    if(typeof sails !== 'undefined') {
      if(typeof sails.config !== 'undefined' && typeof sails.config.waterlock !== 'undefined'){
         var sailsConfig = sails.config.waterlock.jsonWebTokens;
         configData = _.merge(configData, sailsConfig, function(a, b) {
             if (_.isArray(a)) {
                return a.concat(b);
             }
         });
   ...
   *
   * @param  {Object} req the express request object
   * @param  {Object} optional - the name of the property to pull the token from
   * @return {String} token
   * @api public
   */
  getAccessToken: function(req, tokenName){
    var token = null;
    if (req.headers && req.headers.authorization) {
      ...
      if (tokenName) {
        token = this.allParams(req)[tokenName];
      }
      else {
